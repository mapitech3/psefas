...
def Polygon_To_Line(Polygon,New_Line):
...
def Get_Polygon_vertx(layer,out_put = ''):
        ...
        oid_fieldname   = str(arcpy.Describe(layer).OIDFieldName)
        add_field         (out_put,'Polygon_ID')

        with arcpy.da.SearchCursor (layer, ['SHAPE@',oid_fieldname])as cursor:
                for row in cursor:
                    geom = row[0]
                    for item in geom:
                        for pt in item:
...
def CreateWorkingGDB(gdb,Folder,copy,fc_name):
    ...
    for fc in copy:
        try:
            ...
            copy_me = get_fc_from_mxd        (fc_name)
...
def generateCurves(fc):
    ...
    for row in arcpy.SearchCursor(fc):
        ...
        if 'curve' in str(j):
            ...
            pts = [arcpy.Point(f[0], f[1]) for i in coords if i for f in i if f] 
...
def fix_tolerance(layer_path,border):
    ...
    Layer_Management(layer_path).Multi_to_single()
