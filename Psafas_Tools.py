...
Get_Status_Field
delete_parcel_problems     # Not in used
'''
...
def CheckIfSkipProcess(PARCEL_ALL_EDIT,PARCELS_inProc_edit,gdb):
    ...
    list_ref = [[i.OBJECTID,round(i.SHAPE_Area,1)] for i in arcpy.SearchCursor(PARCEL_ALL_EDIT)]
    for n in list_ref:
        if dic.has_key(n[0]):
            if n[1] == dic[n[0]]:
                pass
...
def PrePare_Data(parcel_bankal,parcels_copy,points_copy,Point_bankal,GDB,name_bankal,name_tazar):
    ...
    arcpy.Select_analysis                      (parcel_Bankal_cut,parcel_Bankal_cut + '_aFater_Move_Vertices_By_Name') # DELETEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE
...
def connect_parcel_to_sett(layer,sett,sett_fields = ['SETL_CODE','SETL_NAME']):
    ...
    if sett != '':
        ...
        data = [[i[0],i[1],i[2]] for i in arcpy.da.SearchCursor('sett_layer',sett_fields)]
...
def delete_parcel_problems(layer,tazar):

    '''
    [info] -  מוחק את כל החלקות שקיבלו שם זהה לחלקות התצר, (כתוצאה מהקשתות החיצוניות) ואינם קשורים, 
    INPUT - 
    1) - שכבת הרצף
    2) - תצ"ר המודד
    '''

    Delete_polygons(layer,tazar)
    ...
    for raw in icursor:
        geom = raw.shape
        key = str(raw.GUSH_NUM)+'-'+str(raw.GUSH_SUFFIX)+'-'+str(raw.PARCEL)
        if key in data_tazar:
            icursor.deleteRow(raw)

    arcpy.Append_management(tazar,layer,'NO_TEST')
...
def Update_Layer_Curves_By_ID(fc,tazar,curve):
    ...
    # הוצאת כל הקשתות שיש להם שיוך לחלקת בנקל
...
def delete_Line_by_polygon(AOI_Line,tazar_border,Dissolve = False):
...
def Create_Line_AOI(AOI_final,tazar_border,curves,lines_bankal,lines_modad,AOI_Line):

    Temp_AOI      = r'C:\Users\medad\Desktop\Mpy\Scratch1\Tazar_89705.gdb\Temp_AOI'
    Diss_Line     = r'in_memory\Diss_Line'
    diss_AOI      = r'in_memory\diss_AOI'
    Buffer_AOI    = r'in_memory\Buffer_AOI'
    tazar_buffer  = r'in_memory\tazar_buffer'
    buffer_1000   = r'in_memory\buffer_1000'

    Layer_Management(lines_bankal).Select_By_Location('INTERSECT',AOI_final,0,AOI_Line)

    layer1 = Layer_Management    (AOI_Line)
    layer1.Select_By_Location    ('COMPLETELY_WITHIN',AOI_final)

    delete_Line_by_polygon   (AOI_Line,tazar_border)

    Multi_to_single          (AOI_Line)
    arcpy.Append_management  (lines_modad,AOI_Line,'NO_TEST')

    del_line_Not_on_parcels  (AOI_Line,AOI_final)

    Polygon_To_Line          (AOI_final,Temp_AOI)

    Layer_Management         (Temp_AOI).Select_By_Location('BOUNDARY_TOUCHES',tazar_border,0,None,'invert')

    delete_Line_by_polygon   (Temp_AOI,tazar_border)

    Multi_to_single          (Temp_AOI)
    delete_Line_by_polygon   (Temp_AOI,curves)

    Multi_to_single          (Temp_AOI)
    delete_Line_by_polygon   (Temp_AOI,AOI_Line,True)

    arcpy.Buffer_analysis    (AOI_final,Buffer_AOI,250,'OUTSIDE_ONLY','','ALL')

    delete_Line_by_polygon   (Temp_AOI,Buffer_AOI)

    arcpy.Append_management  (Temp_AOI,AOI_Line,'NO_TEST')

    arcpy.Delete_management  (Temp_AOI)

    Delete_Duplic_Line       (AOI_Line)
